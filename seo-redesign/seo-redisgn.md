...
**Implementováno (1.2, 10. 7. 2026, UX doladění večer 10. 7.):**
- Plný obsah dnešního vydání na `/`; **titulek dne je H2 nahoře** (hlavní vizuální nadpis); canonical homepage na sebe (`https://poslusnehlasim.cz/`)
- **H1 (stabilní, SEO):** `Poslušně hlásím, satirický deník Poslanecké sněmovny` až **dole pod „Celý archiv →"** ([`homepage-archive.html`](../svejk/templates/homepage-archive.html)); slovo **Poslušně hlásím** tučně (`h1_lead` / `h1_tail` v [`strings.json`](../svejk/strings.json))
- Sekce **„Z archivu"** — 5 předchozích vydání + odkaz na celý archiv
- Masthead „Poslušně hlásím!" je dekorace (`<p>`), ne H1
- Úvodní odstavec pod H1 z původní specifikace **nebyl nasazen** (záměrně, aby hlavička nebyla přeplněná)
