...
from svejk.build.io import append_jsonl, write_jsonl
...
def _fetch_steno_to_jsonl(
    ...
    *,
    ...
    for record in client.iter_steno(obdobi, schuze, max_records=max_records):
        append_jsonl(path, _steno_row(record))
        count += 1
        if verbose and (count == 1 or count % 10 == 0):
            tema = (record.tema or "(bez tematu)")[:55]
            print(
                f"  [{count}] por. {record.poradi} ({time.monotonic() - t0:.0f}s) — {tema}",
                file=sys.stderr,
                flush=True,
            )
    ...
    if verbose:
        print(
            f"Steno hotovo: {count} zaznamu za {time.monotonic() - t0:.0f}s.",
            file=sys.stderr,
            flush=True,
        )
    return count
...
def run_fetch(
    ...
    *,
    ...
    if skip_steno:
        ...
        client = HlidacClient(HLIDAC_TOKEN)
